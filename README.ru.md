...
**Copyright (c) 2026 Grigory Shmykov. Все права защищены.** Материалы предоставлены только для оценки портфолио и профессиональных навыков, включая AI-анализ на условиях лицензии. Копирование, запуск, изменение, распространение и использование кода требуют письменного согласия автора, с учётом ограниченного разрешения на оценку и исключений платформы/закона в [LICENSE.md](LICENSE.md). Разрешение на обучение моделей не предоставляется.
...
Существующий том не переинициализируется: простая замена пароля в `.env` не меняет пароль в PostgreSQL. Для существующей установки сохраните текущие значения или выполните отдельную ротацию ролей.
