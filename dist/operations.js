...
function protocolTable(rows){
 ...
 return `<div class="table-wrap"><table><thead><tr>${['Протокол','Заказ','Серийный номер','Время','Результат','Метод / версия','Измерения'].map(x=>`<th>${opText(x)}</th>`).join('')}</tr></thead><tbody>${rows.map(r=>`<tr><td><button type="button" class="order-link protocol-link" data-protocol="${esc(r._id)}" aria-haspopup="dialog"><bdi>${esc(r._id)}</bdi></button></td><td>${evidenceOrderLink(r.order_id)}</td><td><bdi>${esc(r.serial)}</bdi><small>${opText('Версия конфигурации')}: ${esc(r.configuration_version)}</small></td><td>${healthDate(r.tested_at)}</td><td><span class="badge ${r.result==='passed'?'paid':'overdue'}">${opText(r.result==='passed'?'Тест пройден':'Тест не пройден')}</span></td><td>${esc(r.method)} / ${esc(r.method_version)}</td><td>${(r.measurements||[]).map(m=>`<div><bdi>${esc(m.name)}: ${esc(m.value)} ${esc(m.unit)} [${esc(m.min??'—')}…${esc(m.max??'—')}]</bdi></div>`).join('')}</td></tr>`).join('')}</tbody></table></div>`;
}
...
function protocolDetailMarkup(r){
 ...
 <p><span class="badge ${r.result==='passed'?'paid':'overdue'}">${opText(r.result==='passed'?'Тест пройден':'Тест не пройден')}</span></p>
